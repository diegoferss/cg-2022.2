...
namespace cg {
    ...
    TriangleMesh* MakeSpiral(
        Generatrix& generatrix, int num_subdiv, float initial_radius,
        float num_revolutions, float height_inc, float radius_inc,
...
#pragma warning(disable : 6385)  // buffer overrun?????
        ...
        if (draw_back_cap) {
            ...
            *cap_normals++ = cap_normal;
            for (int i{}, tidx{ cap_center + i }; i < tmp.Sides(); i++, tidx++) {
                ...
                *cap_triangles++ = {
                    cap_center, tidx + 1,
                    (i == tmp.Sides() - 1 ? num_subdiv * tmp.Sides() : tidx + 2) };
            }
        }
    ...
    TriangleMesh* MakeTwist(Generatrix& generatrix, int num_subdiv,
        ...
        generatrix.TwistScale(initial_scale);
